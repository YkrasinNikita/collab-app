...
import { FiFileText, FiUser, FiLogOut, FiPlus, FiTrash2, FiClock } from 'react-icons/fi';
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-gray-100">Мои документы</h1>
        <div className="flex items-center gap-3">
          <ThemeToggle />
          <button onClick={() => router.push('/profile')} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 transition">
            <FiUser size={18} />
          </button>
          <button onClick={handleLogout} className="p-2 rounded-full hover:bg-red-50 dark:hover:bg-red-900/30 text-red-500 transition">
            <FiLogOut size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6">
        <button
          ...
          className={`px-4 py-2 rounded-lg font-medium transition ${
            tab === 'documents' ? 'bg-blue-500 text-white shadow-md' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'
          }`}
        >
          Мои документы
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-lg font-medium transition ${
            tab === 'invitations' ? 'bg-blue-500 text-white shadow-md' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'
          }`}
        >
          Приглашения
        </button>
      ...
      {tab === 'documents' ? (
        <>
          <div className="flex flex-wrap gap-2 mb-6">
            <button onClick={() => openCreateModal('document')} className="flex items-center gap-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-4 py-2 rounded-lg shadow hover:shadow-md transition">
              <FiPlus /> Заметка
            </button>
            <button onClick={() => openCreateModal('mindmap')} className="flex items-center gap-2 bg-gradient-to-r from-purple-500 to-pink-600 text-white px-4 py-2 rounded-lg shadow hover:shadow-md transition">
              <FiPlus /> Ментальная карта
            </button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <h2 className="text-lg font-semibold mb-3 text-gray-700 dark:text-gray-300">Заметки</h2>
              {documents.length === 0 && <p className="text-gray-500 dark:text-gray-400">Нет заметок</p>}
              {documents.map(d => <DocumentCard key={d._id} item={d} type="document" />)}
            </div>
            <div>
              <h2 className="text-lg font-semibold mb-3 text-gray-700 dark:text-gray-300">Ментальные карты</h2>
              {mindmaps.length === 0 && <p className="text-gray-500 dark:text-gray-400">Нет ментальных карт</p>}
          ...
          <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Создать новый документ">
            ...
            <div className="flex gap-2">
              <button onClick={handleCreate} className="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-2 rounded-lg font-medium">Создать</button>
              <button onClick={() => setModalOpen(false)} className="flex-1 bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-500 py-2 rounded-lg font-medium">Отмена</button>
            </div>
          </Modal>
          <Modal open={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} title="Удалить элемент?">
            ...
            <div className="flex gap-2">
              <button onClick={confirmDelete} className="flex-1 bg-red-500 hover:bg-red-600 text-white py-2 rounded-lg font-medium">Удалить</button>
              <button onClick={() => setDeleteModalOpen(false)} className="flex-1 bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-500 py-2 rounded-lg font-medium">Отмена</button>
            </div>
          </Modal>
        </>
      ) : (
