...
interface Props {
  onAccepted?: () => void; // новый пропс
}

export default function InvitationsList({ onAccepted }: Props) {
  ...
  if (!invites.length)
    return <p className="text-gray-500">Нет новых приглашений</p>;
        ...
        <div key={inv._id} className="border p-3 rounded flex justify-between items-center">
          <div>
            <p className="font-medium">
              {inv.documentTitle || inv.documentId}
            </p>
            ...
            {typeof inv.fromUser === 'object' && inv.fromUser.name && (
              <p className="text-sm">
                От: {inv.fromUser.name} ({inv.fromUser.email})
              </p>
            )}
          </div>
          <div className="flex gap-2">
            ...
            <button
              onClick={() => handleAction(inv._id, 'declined')}
